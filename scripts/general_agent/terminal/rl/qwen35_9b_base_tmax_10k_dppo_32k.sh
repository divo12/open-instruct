...
# DPPO loss-only ablation of qwen35_9b_base_tmax_10k.sh (our proven 4-node GRPO run,
# wandb awkz09t0). IDENTICAL to that script except the policy loss: this adds
# --loss_fn dppo (+ tv divergence, threshold 0.1). Everything else is unchanged
# (32k response / 8k per-turn, 32 prompts x 8 samples, 16 learners + 16 engines,
# SP=4, pool 512, async 4, NO Liger/fp32/save_trainer_logprobs) so the only variable
# vs awkz09t0 is GRPO -> DPPO. "_32k" distinguishes it from the 64k repro variants.
...
uv run python mason.py \
       --cluster ai2/jupiter \
       --image "$BEAKER_IMAGE" \
       --description "SWERL tmax-15k DPPO with Qwen3.5-9B (32k; loss ablation of awkz09t0)" \
       --pure_docker_mode \
    ...
    --output_dir /output \
    --exp_name swerl_qwen35_9b_base_tmax_15k_dppo_32k \
    --local_eval_every 10 \
    --save_freq 20 \
